...
public class PlayService extends Service implements MediaPlayer.OnErrorListener {
			...
			player.setOnPreparedListener(new MediaPlayer.OnPreparedListener() {
				...
				public void onPrepared(MediaPlayer mp) {
					...
					case STEP_PLAY_PAUSE:
						...
						break;
					case STEP_ADD:
						if (status==PlayListHelper.STATUS_PLAYING){
							player.start();
							progressThread.start();				
						}
						sendPosIntent(current_pos);
	...
	private void playadd() {
		if (player != null && player.getCurrentPosition() >= 5000) {
			player.seekTo(60000+player.getCurrentPosition());
			current_progress_position = 60000+current_progress_position;
		}
	}
